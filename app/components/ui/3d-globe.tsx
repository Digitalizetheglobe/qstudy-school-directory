  ...
  return (
    <group ref={groupRef} visible={isVisible}>
      ...
      <group ref={imageGroupRef} position={topPosition}>
        <Html transform center sprite distanceFactor={10} style={{ pointerEvents: isVisible ? "auto" : "none", opacity: isVisible ? 1 : 0, transition: "opacity 0.15s ease-out" }}>
          <div
            className={cn("cursor-pointer overflow-hidden rounded-full bg-neutral-900 shadow-lg transition-transform duration-200", hovered && "scale-125 shadow-xl ring-1 ring-white/50")}
  ...
  return (
    <group ref={groupRef}>
      ...
      {markers.map((marker, index) => (
        <Marker key={`marker-${index}-${marker.lat}-${marker.lng}`} marker={marker} radius={config.radius} defaultSize={config.markerSize} onClick={onMarkerClick} onHover={onMarkerHover} />
      ))}
    </group>
  );
}
...
function Scene({ markers, config, onMarkerClick, onMarkerHover }: { markers: GlobeMarker[]; config: Required<Globe3DConfig>; onMarkerClick?: (marker: GlobeMarker) => void; onMarkerHover?: (marker: GlobeMarker | null) => void }) {
  const { camera } = useThree();
...
function LoadingFallback() {
  return (
    <Html center>
      <div style={{ color: '#94a3b8', fontSize: '0.85rem' }}>Loading globe...</div>
    </Html>
  );
}
